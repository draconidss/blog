...
由`ClassLoader`负责，将class字节码文件加载到内存中

- 通过一个类的全限定明获取定义此类的二进制字节流；
- 将这个字节流所代表的的`静态存储结构（静态变量，静态代码块，常量池等）`转化为`方法区的运行时数据`；
- 在内存中生成一个对应这个类的`java.lang.Class`对象（用于反射），作为方法区这个类的各种数据的访问入口
...
- 为类变量（static变量）分配内存并且设置该类变量的默认初始化值，具体赋值在初始化阶段完
